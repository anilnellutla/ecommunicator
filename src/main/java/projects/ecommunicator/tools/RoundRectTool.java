...
/**
 * Class that extends class RoundRect and draws RoundRectangle on the whiteboard canvas by 
 * getting the starX and startY on the mouse events of WhiteBoardCanvas for furthur reference 
 ...
 * @version 1.0
 */
public class RoundRectTool extends RoundRect {
	...
	public void drawComponent(Graphics g) {
		...
		if (x1 != 0 && y1 != 0) {
			...
			startX = x1;
			startY = y1;
			//method that draws the round rect on the whiteBoardCanvas						
			draw(g);
			startX = tempX;
			startY = tempY;
		}
	...
	/**
	 * Keep on tracking endX,endY on mouseDragged and draw RoundRect using the
	...
	//fetch endX,endY on mouseReleased and draw RoundRect using the current co-ordinates by repaint()
	...
	/**
	 * method that draws RoundRect by accepting graphics,co-ordinates startX,startY,height,width and 
	 * thickness on the WhiteBoardCanvas on mouseRelesed
	...
	public void draw(Graphics g) {
		...
		g2.setColor(color);
		g2.draw(
			new RoundRectangle2D.Double(startX, startY, width, height, 10, 10));
		/*
	...
	public String getToolString() {
		...
		String toolString =
			Utility.convertTo4Byte(ScoolConstants.ROUND_RECT_TOOL)
