...
/**
 * Class that extends class Rect and draws Rectangle on the whiteboard canvas by getting the 
 ...
 * @version 1.0
 */
public class RectTool extends Rect {
	...
	public void drawComponent(Graphics g) {
		...
		if (x1 != 0 && y1 != 0) {
			...
			startX = x1;
			startY = y1;
			//method that draws the rect on the whiteBoardCanvas			
			draw(g);
			startX = tempX;
			startY = tempY;
		}
	...
	/**
	 * Keep on tracking endX,endY on mouseDragged and draw Rectangle using the
	...
	//fetch endX,endY on mouseReleased and draw Rectangle using the current co-ordinates by repaint()
	...
	/**
	 * method that draws Rectangle by accepting graphics,co-ordinates startX,startY,height,width and 
	...
	public void draw(Graphics g) {
		...
		//g2.fill(new Rectangle2D.Double(startX, startY, width, height));
	}
	...
	public String getToolString() {
		...
		String toolString =
			Utility.convertTo4Byte(ScoolConstants.RECT_TOOL)
